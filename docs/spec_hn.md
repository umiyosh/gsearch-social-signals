...
- Google 検索結果の各リンクに対し、**Hacker News における URL の投稿総数（nbHits）をバッジ表示**する。
- 既存の「はてなブックマーク件数バッジ」と同様に、**検索結果ごとに小さい数値バッジを重ねる UI** を実現する。
- HN のスコアやコメント数 (`points`, `num_comments`) は、**内部的には取得してよいが UI 表示は最小限（nbHits 中心）** にとどめる。
...
- Google 検索結果の各リンクに対し、次のような UI を追加：
  - 既存：`★ 123 users`（はてな）
  - 新規：`HN 12 posts`（Hacker News）

- 表示ルール：
  - `nbHits > 0` の場合のみ表示。
  - 0 件の場合は、HN バッジを出さない（既存の「ブクマ0件は非表示」と同じ思想）。
...
- HN バッジのテキストフォーマット（Phase 1）：
  - 基本形：`HN <nbHits> posts`
  - 将来拡張を見越し、内部データ構造では `nbHits`, `maxPoints`, `maxComments` 等も保持しておく。

- バッジのクリック：
  - 初期実装では **HN 検索画面（Algolia HN Search）への遷移**とする：
    - 例：`https://hn.algolia.com/?query=<encoded_url>&type=story`

  - 将来の Phase 2 で、最もスコアの高い story の HN 本家 URL (`https://news.ycombinator.com/item?id=<objectID>`) へ飛ばすなどの拡張を検討。

### 3.2 HN 言及数の定義

- **Hacker News 上で「その URL が story として投稿された回数」** を言及数とみなす。
- これは Algolia HN API の `nbHits` に対応する。
  - `nbHits` は `query` にマッチしたヒットの総数であり、URL で検索した場合は、その URL にマッチした story 件数となる。
...
- 主要フィールド：
  ...
  - `hits: Array<{ points, num_comments, created_at, url, objectID, ... }>`
    → Phase 1 では集計のみ行い、UI には基本的に出さない（将来に備えてデータ構造には保持）。

- 内部で計算しておくとよい代表値（Phase 2 以降用）：
...
4. background から `RESPONSE_HN_SUMMARY` を受け取ったら、
   ...
   - `nbHits > 0` の場合、該当要素付近に HN バッジ要素を挿入。
...
- HN API レスポンスで `hits` が空、もしくはすべて `points: null` の場合：
  - `nbHits` が 0 であればバッジ非表示。
  - `nbHits` > 0 だが `points: null` が多いケースはあり得るが、Phase 1 では `nbHits` のみ利用するため特別な処理は不要。
...
6. content script にて、Hatena の処理フローを参考にしつつ、**HN バッジを DOM に追加するロジック**を実装すること。
   - `nbHits > 0` のときだけ `HN <nbHits> posts` を表示。
