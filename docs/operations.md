...
- Google / DuckDuckGo SERP layout broken: `src/content/searchResults.ts` またはバッジ配置 CSS の問題を疑う。
...
- Badge not displayed: 対応検索サービス、URL 抽出、message handling、API 結果を確認する。
...
## 検索サービスの SERP DOM 変更時の対応手順

1. 報告された検索サービス、ドメイン、検索語が公開して問題ない場合、同じ条件で再現する。
2. 対象の検索結果コンテナを確認し、Google layout または DuckDuckGo の `data-layout` / `data-testid` がどう使われているかを見る。
...
5. 新しい layout、広告・内部導線除外、redirect URL 正規化、二重挿入防止の回帰テストを追加する。
6. 次を実行する。
...
- 対応している Google と DuckDuckGo の通常Web検索結果ページを開く。
...
- 継続スクロールで後から追加した検索結果に、バッジが二重挿入されないことを確認する。
- DuckDuckGo の広告 (`data-layout="ad"`) と関連検索にバッジが出ないことを確認する。
...
- Google / DuckDuckGo 検索結果の見た目が大きく変わったときの SERP smoke checklist。
